...
/**
  * @brief  注册曲线
  * @param  u8Channel 通道, eWidget 控件枚举
  * @retval true 成功, false 失败
  */
bool HMI_RegisterCurveVar(const uint8_t u8Channel, const HMI_Widget eWidget)
{
	...
	static uint16_t u16Channel = 0;
	static ScrCmdInfo sCmd = {0};
	ScrCmdInfo *pCmd = NULL;
	
	switch (g_sHMIControlManager.u8RegisterCurveStep)
	{
		case SW_PREPARE:
			pCmd = FindCmdPointer(eWidget);
			if (pCmd)
			{
				u16Channel = u8Channel;
				sCmd.eCmd = SYS_CMD_REG_CURVE;
				sCmd.u16VarAddr = pCmd->u16VarAddr;
				sCmd.eDataType = TYPE_UINT16;
				sCmd.u8DataLen = 1;
				sCmd.pData = &u16Channel;
				sCmd.bSuccess = false;
				g_sHMIControlManager.u8RegisterCurveStep = SW_SEND_CMD;
			}
			break;
			
		case SW_SEND_CMD:
			SendCmd(&sCmd);
			g_sHMIControlManager.u8RegisterCurveStep = SW_WAIT_RESULT;
			break;
		...
		case SW_WAIT_RESULT:
			...
			{
				sCmd.bSuccess = false;
				g_sHMIControlManager.u8RegisterCurveStep = 0;
				bRet = true;				
			}
			break;
	}
